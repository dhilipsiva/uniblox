...
- **No cross-platform determinism.** Receivers never re-simulate others' entities (they apply replicated state and interpolate), and prediction only touches entities you own — so no two machines must agree on a float. This is deliberate and is the reason lockstep was rejected (§3). Do not introduce any mechanism that requires browser/x86/ARM peers to compute identical float results. **Accepted consequence — a quality ceiling, not a bug (ADR-0026):** because neither of two *remotely-owned* entities is predicted, each observer sees BOTH through interpolation, so remote-vs-remote interactions carry inherently higher latency (~one interpolation delay + RTT). Do not "reduce the lag" by re-simulating others locally — that reintroduces the rejected determinism (the `netcode-auditor` flags re-simulation of others). Keep Mode-2 cross-owner interactions coarse; precise/competitive play is reserved for the authoritative Mode 3. (This is the interpolation-latency sense of "cross-owner" — distinct from ADR-0025's ownership-arbitration "cross-owner interaction rules".)
...
**Three ceilings to accept, never "fix":** (the first two are anti-cheat ceilings; the third is a netcode-QUALITY ceiling that shares the same "accept, don't fix" disposition and the same envelope)
...
- **Cross-owner interactions are laggy** (a netcode-quality ceiling, not anti-cheat; §2 / L28, ADR-0026). Two remotely-owned entities are both interpolated on every observer and predicted by no one, so remote-vs-remote interactions carry inherently higher latency. Never "close" it by re-simulating others locally (that reintroduces the rejected cross-platform determinism); keep such interactions coarse and reserve precise/competitive play for Mode 3.
