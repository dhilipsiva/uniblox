...
- **Opt-in content-addressed save — C1 (B1–B4 landed):** the primitives + all three stores are DONE — B1 `ContentId`+blake3 (**ADR-0032**), B2 `save_world`/`load_world`/`load_world_verified` + `MemoryStore` (**ADR-0033**), B3 native durable `FileStore` (**ADR-0034**), B4 browser durable `IdbStore` (IndexedDB, async — **ADR-0035**; browser durability verified via the client's `[uniblox-idb]` self-test). Remaining: **C1** — an opt-in save/load keybind in the playable client that serializes the actual game world (`save_world`) → `IdbStore` and restores it on demand/at startup (a save-slot index, since content ids aren't human-memorable). *Acceptance:* in the browser, a save keybind persists the live Mode-1 world and a load restores it — surviving a page reload — by content ID.
