...
#### 1.1 Project + build + AI-workflow scaffolding [LOW — delegate]
- Initialize the Cargo workspace with the crates above (stubs). *Acceptance:* `cargo build` and `cargo test` succeed.
- Set up the **two-WASM-build** pipeline: a WebGPU build and a WebGL2 build, plus JS capability detection to select the build. Bevy cannot serve both from one binary (issue #13168); enabling `webgpu` overrides `webgl2`, and WebGPU requires `RUSTFLAGS=--cfg=web_sys_unstable_apis`, so two separate `cargo build --target wasm32-unknown-unknown` invocations are required. *Acceptance:* both artifacts produced; a stub page loads the correct one per browser capability.
- Add the size-optimized release profile (`opt-level="z"`, `lto=true`, `codegen-units=1`, `strip=true`, `panic="abort"`) and a `wasm-opt -Oz` + brotli post-step (run wasm-opt on the *final* file after wasm-bindgen). *Acceptance:* the build step prints sizes for both builds.
...
- Stand up **CLAUDE.md + .claude/** for the AI workflow: root `CLAUDE.md`, per-crate `CLAUDE.md` stubs, the context files (`PROJECT_STATE.md`, `DECISIONS.md`/ADR log), the **four subagents** (test-writer, netcode-auditor, sandbox-auditor, reviewer) with their tool restrictions, and the **slash commands** (`/build-wasm`, `/slice-check`, `/review-netcode`, `/new-crate`). *Acceptance:* each subagent is invokable; each slash command runs.
- Wire the **hooks**: `cargo fmt` on write; a blocking `cargo clippy --all-targets -- -D warnings` + `cargo test` gate; a **PreToolUse deny** of destructive commands (`rm -rf`, force push, `DROP TABLE`); and a hook that **blocks edits to `tests/` during implementation turns**. *Acceptance:* clippy gate blocks on warnings; a destructive command is denied; an edit to `tests/` during an implementation turn is blocked.
- Configure the **MCP servers**: GitHub, read-only Postgres, a docs/Context7-style server (to counter hallucinated Bevy/matchbox/str0m APIs), and Playwright. *Acceptance:* each server is reachable from a session.
- Feature-prune Bevy via its **`2d`/`3d`/`ui` cargo feature collections** (verify exact collection names against Bevy 0.19 docs) rather than hand-listing features, and add **`wasm-opt -Oz --converge`** (iterate to a fixed point) to the existing opt-level=z/lto/codegen-units=1/strip/brotli stack. *Acceptance:* the build prints size deltas from feature-pruning and from `--converge`.
- Investigate routing audio through a **Web Audio worklet** (runs on its own audio thread, needs no COOP/COEP cross-origin isolation) to decouple audio from main-thread simulation stalls, rather than accepting single-thread audio stutter unconditionally. *Acceptance:* audio does not glitch when the main thread stalls for a frame.
