...
- Wire the claim/commit/reject handshake into `server::net_pump` and CLOSE the cross-owner hardening the ADR-0025 A arbitration primitive left deferred (auditor carry-forwards): (a) **push/pull mutual exclusion** per entity — `transfer_ownership` (owner push) and `claim_ownership`/`drain_commits` (coordinator pull) mint the `OwnerSeq` INDEPENDENTLY and collide at equal `seq` (the tiebreak favors the higher-id minter, so a concurrent push can silently override a granted claim → a transient/again-permanent double-authority window); an entity must use ONE mechanism at a time, or Mode 2 routes ALL ownership changes through the coordinator (sole minter). (b) **Consistent-membership consensus** — exactly-one-coordinator AND Stage B's exactly-once reassignment rely on an agreed `peers` view; a persistent split (two self-elected coordinators oscillating) is NOT bounded by the equal-seq tiebreak. The deferred `net_pump` Disconnected/`track`/`untrack` wiring owns (b). The claim/commit/reject primitive itself (RTT-cost freeze window, no-authority-pre-commit, reliable-channel rejection + retransmit) is DONE (ADR-0025 A-handshake). *Acceptance:* the pump drives claims end-to-end; a concurrent push+pull on one entity converges to a single owner; a membership split is detected and resolved to one coordinator.
