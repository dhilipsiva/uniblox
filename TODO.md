...
- Double-ownership arbitration — the claim/commit/reject PULL path (ADR-0025 A-handshake). DONE so far: the owner-drop reassignment (ADR-0025 B — `reassign_orphans`, lowest-peer-ID `elect_owner`, closed the E4 orphan) AND the **A-kernel `OwnerSeq` gate** (ADR-0025 A-kernel — a per-entity `OwnerSeq{seq,coordinator}` seeded `{0,spawner}`, WIRE 4→5, rides `OwnershipTransfer`+`ResyncSpawn`; the transfer apply gate is now `seq > rec.owner_seq` REPLACING `owner!=from`, so the R6 reorder resolves by rank at the source with no freeze; `ResyncSpawn` is seq-gated too, closing the stale-former-owner backdoor). REMAINING: the coordinator-arbitrated PULL handshake — new `NetEvent::{ClaimOwnership, OwnershipCommit{seq}, ClaimRejected}`; `claim_ownership` sends `ClaimOwnership` to the coordinator = `elect_owner(peers ∪ local)` (flips NO owner); the coordinator collects claims and `drain_commits` mints `{prev.seq+1, coordinator:local}`, emitting `OwnershipCommit` to the winner (= `elect_owner(claimants)`) + `ClaimRejected` to losers; the commit apply gate reuses the kernel's strict `seq >` rule; a claimant assumes authority ONLY on receiving its own commit. Also hardens Stage B's exactly-once under an inconsistent membership view. *Acceptance:* two simultaneous claims resolve to one committed owner by coordinator sequence number, the loser explicitly rejected, no authority assumed pre-commit.
