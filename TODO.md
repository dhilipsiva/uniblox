...
#### 1.1 (remaining) Build-pipeline execution + Bevy feature-prune + MCP + audio [LOW — delegate; blocked on tooling]
Scaffolding for all of the below is **committed and verified**: the `crates/*` Cargo workspace (9 stubs, `cargo build`/`cargo test` green) + the size-optimized `[profile.release]` (`opt-level="z"`, lto, codegen-units=1, strip, panic="abort") + the single-threaded stance (no COOP/COEP anywhere); the full `.claude/` AI workflow — per-crate `CLAUDE.md`, `PROJECT_STATE.md`, `DECISIONS.md` (ADR log), the four subagents (test-writer, netcode-auditor, sandbox-auditor, reviewer) with tool restrictions, five slash commands (`/build-wasm`, `/slice-check`, `/review-netcode`, `/new-crate`, `/write-tests`), and the four hooks (`.claude/settings.json` + `scripts/hooks/*` + the `scripts/git-hooks/pre-commit` clippy+test gate); plus the build-pipeline scripts (`scripts/build-wasm.sh`, `scripts/slice-check.sh`, `scripts/serve.sh`), the capability-detection page (`crates/client/web/index.html`), and the `.mcp.json` scaffold. The scripts fail gracefully until their prerequisites exist. What remains needs prerequisites absent at the stub stage:
- Install the WASM toolchain (`wasm-bindgen`, `wasm-opt`, `brotli`, `twiggy`) and — once the Bevy client renders (Phase 1.3–1.6) — run `scripts/build-wasm.sh` to PRODUCE the two artifacts (WebGPU: `--features webgpu` + `RUSTFLAGS=--cfg=web_sys_unstable_apis`; WebGL2: default), each via wasm-bindgen → `wasm-opt -Oz --converge` → brotli, and confirm `crates/client/web/index.html` loads the correct bundle per `navigator.gpu`. *Acceptance:* both artifacts produced; the page loads the right one; the build prints raw→bindgen→wasm-opt→brotli sizes for both builds.
...
- Bring the four **MCP servers** up (GitHub, read-only Postgres, docs/Context7-style to counter hallucinated Bevy/matchbox/str0m APIs, Playwright) — needs `node`/`npx` installed in WSL, a running read-only Postgres role, a GitHub PAT in `.claude/settings.local.json` (never in the tracked `.mcp.json`), and Playwright browsers. *Acceptance:* each server is reachable from a session.
