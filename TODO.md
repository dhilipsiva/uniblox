...
- Interest-management follow-ups (ADR-0021 delivered the AOI gate + read-cheat withholding). Remaining: (a) a **shared per-tick snapshot** so `collect_all` builds the grid/query ONCE instead of re-encoding per peer (perf); (b) **AOI-flicker hysteresis** (two radii) so an entity oscillating across the edge doesn't churn Spawn/Despawn every tick; (c) a real per-client **AOI focus** in the server/client (Mode 3 currently leaves AOI unset ⇒ clients see all). *Acceptance:* per-tick sender cost is O(entities + peers·query) not O(peers·entities); edge-oscillation produces no churn; a client sees only its focus radius. **[MIXED — perf + a small gameplay-focus hook.]**
