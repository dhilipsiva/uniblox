...
- **Opt-in content-addressed save (B3–B4 + C1; B1+B2 landed):** B1 (`ContentId`+blake3 in `protocol`, **ADR-0032**) and B2 (`persistence` crate — `SaveBlob` DTO codec + `save_world`/`load_world`/`load_world_verified` + `MemoryStore`, **ADR-0033**) are DONE — save/reload by content ID round-trips **in-memory** (bullet-2's literal acceptance met headlessly). Remaining: B3 native `FileStore` (`std::fs`, cfg-gated); B4 browser `IdbStore` (IndexedDB, async — durable browser save); C1 opt-in save/load keybind wired into the playable client. *Acceptance:* save/reload round-trips through a native file store, and a browser Mode-1 save survives a page reload (IndexedDB).
