...
- Replication throughput follow-ups (deferred optimizations, not correctness): **message splitting** (a per-tick outbox that exceeds one datagram — currently warn-logged at >1150 B — should split across datagrams) and **per-entry ack granularity** (a single persistently-unresolvable entry withholds acks for a sender's whole stream, forcing full re-sends; the F1 all-or-nothing ack is correct but coarse). *Acceptance:* an over-MTU snapshot is split and reassembled; a stuck entry no longer stalls the rest of the stream's baseline advance.
