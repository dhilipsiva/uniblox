...
- Wire anti-entropy resync into the production pump. The mechanism is built + deterministically tested (ADR-0024 — per-peer `Digest` → `ResyncRequest` → privileged `ResyncSpawn`; injected desync self-heals, the R6 frozen-wrong-owner gap heals, no-CRDT), but `server::net_pump` does not yet CALL `collect_resync`/`drain_resync_requests`/`drain_resync_responses` on a cadence. *Acceptance:* net_pump drives resync on a slow virtual-clock cadence (decoupled from the net tick); an injected desync self-heals over the real pump.
