...
**Goal:** the WebSocket service required even for free tiers. Delegate heavily. **DONE (ADR-0037): SDP/ICE signaling + session registry + `{mode, version}` scoping** — `services` (`build_signaling_server` + `SessionRegistry`) wraps matchbox FullMesh with the scope in the room PATH (`<mode>~<engine>.<content>.<schema>~<lobby>`), so FullMesh isolates mismatched peers structurally (offers/answers relay within one room; sessions listed; scoping enforced) + a gate that rejects malformed scopes. This also SUBSUMES "matchmaking groups only same-mode, same-version players" (a different mode/version is a different room ⇒ never matched). Remaining bullets need more than exact path-scoping:
- Version-triple enforcement/gating (`{engine, content, schema}`) with a CLEAR REJECTION REASON at join, and the **asymmetric** filter — admit if client **engine ≥ the game's declared minimum** (engine releases are backward-compatible) but require **content ID and schema version exact**. Exact path-scoping already isolates non-identical versions, but "admit compatible-but-not-identical engines into ONE session" needs the server to GROUP compatible peers (a custom `SignalingTopology` + a compat function) and to reply with a reason (not just a 401). Version-gating on join is the desync defense (engine/binary is a versioned release, not hot-reloadable; content is hot-reloadable at runtime). *Acceptance:* an older-but-compatible engine joins the same session; any content/schema mismatch is rejected with a clear reason.
- Custom `SignalingTopology` with client-specified `?next=N` session-SIZE grouping (matchbox's `?next=N` lives in the un-vendored `matchbox_server`, not the library) — group the next N compatible peers into a capped session. *Acceptance:* `?next=N` forms sessions of ≤N same-scope peers.
