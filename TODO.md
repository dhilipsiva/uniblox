...
- **Opt-in content-addressed save (B2–B4 + C1; B1 landed):** B1 (`ContentId`+blake3 in `protocol`) is DONE — **ADR-0032** (blake3-256 `ContentId([u8;32])` + `content_id()` + hex round-trip + reserved `VersionTriple`). Remaining: B2 `persistence` crate (`SaveBlob` DTO codec + `save_world`/`load_world`/`load_world_verified` + `MemoryStore`); B3 native `FileStore`; B4 browser `IdbStore` (IndexedDB, async — durable browser save); C1 opt-in save/load keybind wired into the playable client. *Acceptance:* save/reload by content ID round-trips (in-memory + native file), and a browser Mode-1 save survives a page reload (IndexedDB).
