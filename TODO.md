...
- **Replication bandwidth per peer** — the pre-delta native baseline is in `/slice-check` (742 B/s @ 2 entities, 20 Hz); the delta win is now proven DETERMINISTICALLY (ADR-0020 two-World T35: 0 steady-state bytes for a confirmed stationary scene). Still to measure over the real transport: the post-delta B/s under motion, post-interest-management (Phase 3), whether 30–60 Hz is affordable, and realistic entity counts.
