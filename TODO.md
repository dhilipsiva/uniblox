...
### PHASE 4 — Mode 1 Standalone: COMPLETE

Delivered the free, local-authority, no-networking Mode-1 runtime + the full content-addressed save
(ADR-0030…ADR-0036; trail in `DECISIONS.md`, live status in `PROJECT_STATE.md`). **Runtime:** the net-free
`standalone` crate (`build_standalone_app` — no transport/replication in its crate graph, `cargo tree`-guarded;
ADR-0030) wired into the browser-playable client (`Camera2d` + keyboard `Avatar` + drifting NPCs; ADR-0031).
**Save:** `ContentId`/blake3 in `protocol` (ADR-0032) → the `persistence` crate `save_world`/`load_world`/
`load_world_verified` + DTO-mirror `SaveBlob` + `MemoryStore` (ADR-0033) → native durable `FileStore` (ADR-0034)
→ browser durable `IdbStore` (IndexedDB, async; ADR-0035) → opt-in client save/load keybinds (`K`/`L`; ADR-0036).
Browser paths (playable render, IndexedDB save/reload) are manually verified — no headless wasm/GPU runner in this
env; the two-WASM size-budget gate stayed PASS (~3.40/3.42 MB brotli).

### PHASE 5 — Central services (signaling / session / matchmaking) [LOW]
**Goal:** the WebSocket service required even for free tiers. Delegate heavily. **DONE (ADR-0037): SDP/ICE signaling + session registry + `{mode, version}` scoping** — `services` (`build_signaling_server` + `SessionRegistry`) wraps matchbox FullMesh with the scope in the room PATH, so FullMesh isolates mismatched peers structurally (offers/answers relay within one room; sessions listed; scoping enforced) + a gate. Subsumes "same-mode, same-version matchmaking". **DONE (ADR-0038): version-triple enforcement/gating — the asymmetric filter** — the scope format became `<mode>~<content>.<schema>~<min>~<lobby>` with the client's own engine moved to a `?engine=N` query (out of the room key), so content/schema/min/lobby stay exact-match structural while compatible-but-newer engines share ONE room; the gate admits iff `engine ≥ min_engine`, else a reasoned rejection (`426` too-old / `400` malformed/missing-engine). An older-but-compatible engine joins the same session; a too-old engine is rejected with a clear reason; content/schema mismatch is structurally isolated. **DONE (ADR-0039): custom `NextTopology` + `?next=N` session-SIZE grouping** — replaced FullMesh with a custom `SignalingTopology` (`SignalingServerBuilder::new`) that re-implements FullMesh's relay GENERALIZED with `?next=N`: absent ⇒ one unbounded session per room, `?next=N` ⇒ the room subdivides into sessions keyed `<room>#<index>` capped at N (batch-deal / no-backfill). `?next` is stashed by the gate → bridged to the `PeerId`; the `SessionRegistry` became the topology's shared state. Sessions of ≤N same-scope peers, spill-to-new-session, no-backfill, cross-session relay isolation all tested. **DONE (ADR-0040): horizontal scale — stateless nodes + a shared Redis session registry** — `SessionRegistry` split into the node-local relay (unchanged) + a shared `RegistryStore` that MIRRORS session→peer membership so many stateless nodes share one listing; async `join`/`leave` mirror to the store (no `MutexGuard` across the `.await`); stores are `MemoryRegistryStore` (default) + `RedisRegistryStore` (redis-rs; SADD/SREM/SCARD + an index SET). Two nodes over one Redis share the registry (proven by a hermetic `redis-server` test + an in-memory two-node test). Only the LISTING is shared — the relay is node-local (a session's peers must be on one node = sticky routing; cross-node relay deferred; atomic MULTI/Lua + TTL crash-cleanup + autoscale are Phase-11). Remaining bullet:
