...
- **Opt-in content-addressed save (B4 + C1; B1+B2+B3 landed):** B1 (`ContentId`+blake3 in `protocol`, **ADR-0032**), B2 (`persistence` — `SaveBlob` codec + `save_world`/`load_world`/`load_world_verified` + `MemoryStore`, **ADR-0033**), and B3 (native durable `FileStore` — `<hex>.blob` files, `std::fs`, cfg-gated, **ADR-0034**) are DONE — save/reload by content ID round-trips in-memory AND through a native file store (durable across restart). Remaining: B4 browser `IdbStore` (IndexedDB, async — durable browser save); C1 opt-in save/load keybind wired into the playable client. *Acceptance:* a browser Mode-1 save survives a page reload (IndexedDB), and a save keybind in the client persists/restores by content ID.
