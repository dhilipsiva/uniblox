...
- Double-ownership arbitration by coordinator sequence number. The orphaned-entity-on-owner-drop half is DONE (ADR-0025 B — host-migration reassignment via `reassign_orphans`, lowest-peer-ID `elect_owner`; also closed the ADR-0024 E4 orphan). REMAINING (design decided, tightly coupled with the timing/commit-protocol bullet below): a full **claim/commit/reject** through a coordinator = lowest-peer-ID (reuse `elect_owner`); a per-entity `OwnerSeq{seq,coordinator}` (lexicographic — higher wins, the `coordinator` field makes coordinator-migration robust) gates EVERY owner-mutating apply (`OwnershipTransfer` AND `ResyncSpawn` — else a backdoor around arbitration); new `NetEvent::{ClaimOwnership, OwnershipCommit{seq}, ClaimRejected}` (WIRE_VERSION 4→5); a claimant assumes authority ONLY on receiving its own commit. Also hardens Stage B's exactly-once under an inconsistent membership view. *Acceptance:* two simultaneous claims resolve to one committed owner by coordinator sequence number, the loser explicitly rejected, no authority assumed pre-commit; the R6 cross-sender reorder resolves by seq with no freeze.
