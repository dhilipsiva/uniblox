...
**Goal:** free, local-authority, no networking, no anti-cheat. Scope (planning session): runtime = headless + browser-playable; save persists via browser IndexedDB. The headless runtime landed — **ADR-0030** added the net-free `standalone` crate (`build_standalone_app`, no transport/replication in its crate graph; a `cargo tree` guard enforces it), closing the "runs with the networking stack absent" acceptance headlessly.
- **A2 — browser-playable Mode 1:** wire the net-free `standalone` sim into the client's `DefaultPlugins` render app — spawn the sim entities + a `Camera2d`, `standalone::add_sim_systems` on FixedUpdate, keyboard input → avatar `Velocity`, `Position` → sprite `Transform` (mind the `use bevy::ecs as bevy_ecs;` alias). *Acceptance:* in-browser, entities move under `simulate` and arrow/WASD keys move the avatar; the WASM size-budget gate re-checked (≤ ~8 MB brotli/build).
