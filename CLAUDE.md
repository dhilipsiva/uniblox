...
**Phase 1 scaffolding, the Rhai↔Bevy bridge, the mini-game, the transport core, and the custom replication protocol are done.** The Cargo workspace exists (9 crates under `crates/*`), with the size-optimized `[profile.release]`, the AI-workflow scaffolding (`.claude/agents/`, `.claude/commands/`, `.claude/settings.json` hooks, per-crate `CLAUDE.md`, `PROJECT_STATE.md`, `DECISIONS.md`), the build-pipeline scripts, and the Nix flake devShell (ADR-0010). Implemented so far: **`scripting`** (locked-down Rhai engine as a NonSend resource, ADR-0011, sandbox-audited); **`engine-core`** (the authority abstraction — `authority_of` as the single decision point; the Mode-2/Mode-3 swap proven by unit tests) + **`protocol`** (the replication wire format: spawner-stable `NetEntityId`, quantization, reserved signature field); **`transport`** (matchbox 0.14 two-channel wrapper, ADR-0012) + **`services`** (signaling binary); **`replication`** (the HIGH-risk core, ADR-0013 — authority-gated sender, newest-seq LWW receiver, handoff; 27-test battery incl. e2e over real WebRTC, netcode-audited). Deps pinned in `[workspace.dependencies]`: rhai 1.25, bevy_ecs 0.19, matchbox 0.14, postcard/serde, tokio, wasm glue. See `PROJECT_STATE.md` for live status; `DECISIONS.md` for the ADR log. The design is fully specified in `docs/`; next is the authority-swap to Mode 3 [HIGH] — the architecture go/no-go gate.
