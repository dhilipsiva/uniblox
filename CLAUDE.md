...
**Phase 1 scaffolding, the Rhai↔Bevy bridge, the mode-agnostic mini-game, and the transport core are done.** The Cargo workspace exists (9 crates under `crates/*`), with the size-optimized `[profile.release]`, the AI-workflow scaffolding (`.claude/agents/`, `.claude/commands/`, `.claude/settings.json` hooks, per-crate `CLAUDE.md`, `PROJECT_STATE.md`, `DECISIONS.md`), the build-pipeline scripts, and the Nix flake devShell (ADR-0010). Implemented so far: **`scripting`** (locked-down Rhai engine as a NonSend resource, ADR-0011, sandbox-audited); **`engine-core`** (the authority abstraction — `authority_of` as the single decision point, one branching `simulate` system, the Mode-2/Mode-3 swap proven by unit tests) + `protocol::PeerId`; **`transport`** (matchbox 0.14 two-channel wrapper, ADR-0012, hermetic native P2P test green) + **`services`** (full-mesh signaling binary) + the wasm client transport demo. Deps pinned in `[workspace.dependencies]`: rhai 1.25, bevy_ecs 0.19, matchbox 0.14, tokio, wasm glue. See `PROJECT_STATE.md` for live status (incl. the one transport residual: the browser-tab run, blocked in WSL2 headless); `DECISIONS.md` for the ADR log. The design is fully specified in `docs/`; next is the custom replication protocol [HIGH].
